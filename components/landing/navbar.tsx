...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#040316]/85 backdrop-blur-2xl text-white transition-all duration-300">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-full border border-indigo-500/30 bg-indigo-500/10 p-2.5 text-indigo-300 shadow-[0_0_15px_rgba(99,102,241,0.2)] backdrop-blur-md transition duration-300 hover:bg-indigo-500/20 hover:text-white active:scale-95 md:hidden"
          >
            {isOpen ? (
              // Close Icon
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 top-[61px] z-40 bg-[#040316]/95 backdrop-blur-2xl md:hidden flex flex-col justify-between border-b border-white/10 animate-in fade-in slide-in-from-top-4 duration-300 overflow-y-auto px-6 py-8"
          style={{ height: "calc(100vh - 61px)" }}
        >
          {/* Navigation Links List */}
          <div className="space-y-3">
            <div className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 pb-2 border-b border-white/10">
              NAVIGASI UTAMA PROJEKIN
            </div>
            <div className="grid gap-2 pt-2">
              {mobileNavItems.map((item) => (
                <a
                  ...
                  href={item.href}
                  onClick={() => setIsOpen(false)}
                  className="group flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-base font-bold text-white transition-all duration-300 hover:border-indigo-500/50 hover:bg-white/[0.08] hover:shadow-[0_0_20px_rgba(99,102,241,0.2)] active:scale-98"
                >
          ...
          {/* Quick Badges & Direct WhatsApp CTA in Mobile Drawer */}
          <div className="mt-8 pt-6 border-t border-white/10 space-y-4">
            ...
            <a
              ...
              rel="noreferrer"
              onClick={() => setIsOpen(false)}
              className="flex items-center justify-center gap-2 w-full rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-4 text-sm font-black uppercase tracking-wider text-white shadow-[0_0_30px_rgba(16,185,129,0.3)] transition duration-300 hover:scale-[1.02] active:scale-95"
            >
